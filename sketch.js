...
function draw() {
  ...
  if (temp === 1) 
  {
    ...
    text("Press space to restart", 400, 365);
  ...
  if (player.x > 1330 || player.x < 10 || player.y > 720 || player.y < 0) 
  {
    ...
    text("Press space to restart" , 400,330);
     gameState = end;

  }

  //when space is pressed change the gamestate with necessary functions
  if(gameState === end && keyDown(32))
   {
     gameState = play;
